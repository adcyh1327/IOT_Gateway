...
void make_http_response_head(
	...
	unsigned long len	    /**< size of response header */
	)
{
  char * head;
  char tmp[10];
...
void find_http_uri_type(
	unsigned char * type, 	/**< type to be returned */
	char * buf		        /**< file name */
	) 
{
...
void parse_http_request(
	...
	unsigned char * buf				/**< pointer to be parsed */
	)
{
  char * nexttok;
...
unsigned char* get_http_param_value(char* uri, char* param_name)
{
  ...
  uri[content_len]=0;
  /***************/
  if((name = (unsigned char*)strstr(uri,param_name)))
  {
    name += strlen(param_name) + 1; 
    unsigned char* pos2=(unsigned char*)strstr((char*)name,"&");
    if(!pos2) 
    {
